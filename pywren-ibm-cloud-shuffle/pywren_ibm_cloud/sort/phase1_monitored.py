...
def _partition_into_segments_monitored(args, ibm_cos):
    ...
    def _chunk_consumer(t_i):
        ...
        while True:
            ...
            if not correct_types(df, normalized_type_names):
                ...
                del df
                nm = nms[0]
                df = pd.DataFrame(df_partial_columns[nm])
                for nm in nms[1:]:
            ...
            if types[key_nm] not in ['bytes', 'str', 'object']:
                ...
                key_pointer_str['key'] = \
                    np.concatenate([key_pointer_str['key'], df[str(parser_data['column_numbers'][0])]])
                key_pointer_str['pointer'] = \
                    np.concatenate([key_pointer_str['pointer'],
                                    array(range(current_lower_bound, current_shape + current_lower_bound))])
                for nm in nms_without_key:
                    df_columns[nm] = np.concatenate([df_columns[nm], df[nm]])
    ...
    def _chunk_producer(t_i):
        while not q_reads.empty():
            ...
            df = pd.read_csv(read_part['Body'], engine='c', index_col=None, header=None,
                             names=nms,
    ...
    for segm_i in range(1, len(segment_info) + 1):
        ...
        if segm_i == len(segment_info):
            ...
            segm_end = np.searchsorted(ed.iloc[:, int(parser_data['column_numbers'][0])],
                                       (right_bound))
