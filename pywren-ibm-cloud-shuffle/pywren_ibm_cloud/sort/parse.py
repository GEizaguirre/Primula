...
def _parse(info, ibm_cos):
    ...
    total_size = ibm_cos.get_object(Bucket=input_bucket, Key=input_path)['ContentLength']
    info['total_size'] = total_size
    info['low_memory'] = True
    partition_file_path = info['partition_file_path']

    # Infer approximate total rows
    c_b, c_u = _extract_real_bounds_csv(ibm_cos, 0, BOUND_EXTRACTION_MARGIN * 5, total_size, input_bucket, input_path)
    ...
    possible_chunks = range(0, total_size - chunk_size_per_sampler, chunk_size_per_sampler)
    # TODO:
    ...
    selected_sample_chunks = [sample(list(sample_chunk_ranges[i]), num_workers_phase1) for i in range(MAX_READS_PER_SAMPLER)]
    global_ranges = [[s[i] for s in selected_sample_chunks] for i in range(num_workers_phase1)]
    ...
    chunk_bounds = fragment_dataset_into_chunks_csv(num_workers_phase1, total_size)
