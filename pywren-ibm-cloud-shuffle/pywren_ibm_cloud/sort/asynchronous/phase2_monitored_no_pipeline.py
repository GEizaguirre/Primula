...
from pywren_ibm_cloud.sort.asynchronous.config import SLEEP_TIME_ASYNC, MAX_READ_TIME, MAXIMUM_RECONNECTIONS
...
from pywren_ibm_cloud.sort.config import DEFAULT_THREAD_NUM, DATAFRAME_ALLOCATION_MARGIN, \
    PHASE_2_MULTIPLIER, DEFAULT_MULTIPART_NUMBER
            ...
            while True:
                ...
                if types[nms[parser_data['column_numbers'][0]]] not in ['bytes', 'str', 'object']:
                    ...
                    if (current_lower_bound + current_shape) > approximate_rows:
                        ...
                        key_pointer_str['key'] = append(key_pointer_str['key'],
                                                        [extend_value for i in range(current_shape * 3)])
                        extend_value = key_pointer_str['pointer'][0]
                        key_pointer_str['pointer'] = append(key_pointer_str['pointer'],
                                                            [extend_value for i in range(current_shape * 3)])
                ...
                if not is_speculative:
                    _upload_progress(ibm_cos, segment_i, (current_lower_bound) / approximate_rows,
