...
def _partition_into_segments_monitored(args, ibm_cos):
    ...
    my_bucket = bucket_list[int(range_i/workers_p_bucket)]
    my_path = "{}_{}".format(input_path, int(range_i/workers_p_bucket))
    ...
    ##############################################
    lower_bound = partition_ranges[int(range_i%workers_p_bucket)]
    upper_bound = partition_ranges[int(range_i%workers_p_bucket) + 1]
    ##############################################
    lower_bound, upper_bound = _extract_real_bounds_csv(ibm_cos, lower_bound, upper_bound, total_size,
                                                        my_bucket,
                                                        my_path)
    print("lb: {}, up: {}".format(lower_bound, upper_bound))
    ...
    for i in range(len(read_bounds) - 1):
        lb, ub = _extract_real_bounds_csv(ibm_cos, read_bounds[i], read_bounds[i + 1], total_size, my_bucket,
                                          my_path)
        read_bounds[i] = lb
    ...
    def _order_upload_segm_info_granuled(granule_info):
        ...
        segment_bucket = bucket_list[int(s_i/workers_p_bucket)]
        ibm_cos.put_object(Bucket=segment_bucket,
