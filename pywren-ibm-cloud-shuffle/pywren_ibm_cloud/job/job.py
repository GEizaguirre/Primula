...
def create_map_job(config, internal_storage, executor_id, job_id, map_function, iterdata, runtime_meta,
    ...
    if utils.is_object_processing_function(map_function):
        ...
        logger.debug(
            'ExecutorID {} | JobID {} - Calling map on partitions from object storage flow'.format(executor_id, job_id))
...
def _create_job(config, internal_storage, executor_id, job_id, func, data, runtime_meta,
    ...
    # logger.debug("DEV:::_CREATE_JOB->{}".format(inspect.getsource(func)))
    ...
    log_msg = ('ExecutorID {} | JobID {} - uploaded data '
               '- Time: {}'.format(executor_id, job_id, time.time() - data_upload_time))
    print(log_msg) if not log_level else logger.info(log_msg)
    host_job_meta['data_upload_time'] = time.time()
    ...
    # with open("test_fun.pickle", 'wb') as f:
    #     f.write(func_module_str)
    f_upload_time = time.time()

    internal_storage.put_func(func_key, func_module_str)

    log_msg = ('ExecutorID {} | JobID {} - uploaded data '
               '- Time: {}'.format(executor_id, job_id, time.time() - f_upload_time))
    print(log_msg) if not log_level else logger.info(log_msg)
