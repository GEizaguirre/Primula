...
import ibm_boto3
from pywren_ibm_cloud.sort.predict.get_functions import get_cos_functions

from .invoker import FunctionInvoker
from pywren_ibm_cloud.sort.conversion import convert_imzml_csv
from pywren_ibm_cloud.sort.sort import create_sort_imzml, create_sort_csv, SORT_DATA_FORMATS
...
from pywren_ibm_cloud.config import default_config, extract_storage_config, EXECUTION_TIMEOUT, JOBS_PREFIX, \
    default_logging_config
from pywren_ibm_cloud.utils import timeout_handler, is_notebook, is_unix_system, is_pywren_function, create_executor_id

from pywren_ibm_cloud.sort.utils import clear_cos_prefix
...
class FunctionExecutor:
    ...
    def call_async(self, func, data, extra_env=None, runtime_memory=None,
        ...
        # logger.debug("DEV:::CALL_ASYNC->{}".format(inspect.getsource(func)))
    ...
    def sort(self, input_data_path, primary_key_column, format_of_data="csv", segm_n=None, num_workers=None,
             granularity=None, delimiter=None, dtypes=None, speculative_map=True, speculative_reduce=True,
             asynchronous=True, output_path=None, output_bucket=None, segment_ranges=None):

        # Initializations
        if format_of_data.lower() not in SORT_DATA_FORMATS:
            raise ValueError('Input data format not valid')
        if format_of_data == "csv" and primary_key_column is None:
            raise ValueError('For csv files you must specify the primary key column')
        ibm_cos = self.internal_storage
        if num_workers is None:
            num_workers = segm_n
        if format_of_data == 'imzml':
            create_sort_imzml(self, input_data_path, format_of_data, ibm_cos, segm_n, num_workers,
                              granularity=granularity)
        if format_of_data == 'csv':
            create_sort_csv(self, input_data_path, format_of_data, segm_n, primary_key_column,
                            num_workers_p1=num_workers, granularity=granularity, delimiter=delimiter,
                            dtypes=dtypes, speculative_map=speculative_map, output_bucket=output_bucket,
                            speculative_reduce = speculative_reduce, asynchronous=asynchronous,
                            output_path=output_path, segment_ranges=segment_ranges)

    def imzml_to_csv(self, config, input_bucket, input_key, output_bucket, output_key):
        ...
        convert_imzml_csv(self, input_bucket, input_key, output_bucket, output_key, ibm_cos)

    def clean_cos_prefix(self, bucket, prefix):
        args = {'info': {'prefix': prefix, 'bucket': bucket}}
        futures = self.call_async(clear_cos_prefix, args)
        self.get_result(futures)

    def setup_shuffle_model(self, granularity=None):
        key_prefix = get_cos_functions(self.config, granularity=granularity)
        print("Removing intermediate data")
        self.clean_cos_prefix(self.config["pywren"]["storage_bucket"], key_prefix)
